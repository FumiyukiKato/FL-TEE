...
for ((n_users=1;n_users<=25;n_users++)); 
do
    BIG=$(( $n_users*50 + 1500 ))
    bin/bench -v -a optimized -c 10000 -d 201588 -k 2015 -t 2 --sampling_ratio=0.3 --optimal_num_of_clients=$BIG
done

for ((n_users=1;n_users<=25;n_users++)); 
do
    BIG=$(( $n_users*50 + 1500 ))
    bin/bench -v -a optimized -c 10000 -d 50890 -k 5089 -t 2 --sampling_ratio=0.3 --optimal_num_of_clients=$BIG
done